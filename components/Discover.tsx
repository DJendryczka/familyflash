...
const Discover = () => {
  ...
  return (
    <div className=' xl:border-b-2 xl:border-gray-200 pb-6'>
        <div className=' xl:border-gray-500 font-semibold m-3 mt-4 hidden xl:block'>
            Popular Topics
        </div>
        <div className=' flex gap-3 flex-wrap'>
            { topics.map((item) => (
                <Link href={`/?topic=${ item.name}`} key={item.nem}>
                    <div className= { topic === item.name ? activeTopicStyle : topicStyle}>
                        <div className=' font-bold text-2xl xl:text-md'>
                            {item.icon}
                        </div>
                        <div className=' font-medium text-md hidden xl:block capitalize'>
                            {item.name}
                        </div>
                    </div>
